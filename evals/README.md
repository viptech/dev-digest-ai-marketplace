...
| `covers every named requirement, including the vague ones, with an AC-N or an explicit open question (AC-5)` | `agents/spec-creator/spec-creator.cases.ts` | 1.1.0 backward-compatible change (AC-5) | **Ran for real on both 1.0.0 and 1.1.0 `spec-creator.md` — AC-5's "not applicable" branch, honestly**: 1.0.0 scored 5/5, 1.1.0 scored 4/5 on the same fixture — the opposite of a clean discriminating result, and with n=1 per side this is ordinary model-sampling variance (see the case file's own comment for the full account after three escalating fixture attempts). `claude-haiku-4-5` is thorough enough on this fixture that the 1.1.0 gate's effect could not be measured, not because the gate is wrong, but because this specific case doesn't exercise the failure mode reliably at this model's capability level. |
