 ...
 * keeps an "assert before record" bug from creeping in (record() always fires in a `finally`,
 * strictly before the asserts).
 */
...
export type WorkflowCase =
  ...
  | {
      ...
      // of coarser diagnostics and no control run — use contrast when you must isolate the
      // plugin-loading contribution. Every provided expectation must hold; omitted fields are not
      // checked.
      kind: "trace";
      name: string;
      prompt: string;
...
/**
 * Did a skill engage? Either an explicit Skill tool-call, or reading its SKILL.md. `skill` may be
 * bare ("run-plan") or namespaced ("sdd-engineering:run-plan"); a bare name matches a namespaced
 * invocation too (`s.endsWith(":${skill}")`). The file-read check looks for the
 * `.../skills/<skill>/SKILL.md` suffix, which holds regardless of which plugin the skill lives
 * under (`plugins/<plugin>/skills/<skill>/SKILL.md`).
 */
...
type BoundTask = (prompt: string, opts?: RunOptions) => Promise<Result>;

function runQualityCases(cases: QualityCase[], task: BoundTask): void {
  for (const c of cases) {
    test(c.name, async () => {
      const threshold = c.threshold ?? DEFAULT_THRESHOLD;
      const result = await task(c.prompt, { maxTurns: c.maxTurns });
...
/** Run cases against one skill, identified by its owning plugin + name. */
export const runSkillCases = (plugin: string, skill: string, cases: SkillCase[]) =>
  runQualityCases(cases, (prompt, opts) => skillTask(prompt, plugin, skill, opts));

/** Run cases against one agent, identified by its owning plugin + name. */
export const runAgentCases = (plugin: string, agent: string, cases: AgentCase[]) =>
  runQualityCases(cases, (prompt, opts) => agentTask(prompt, plugin, agent, opts));
  ...
  for (const c of cases) {
    test(c.name, async () => {
      if (c.kind === "dispatch") {
        ...
        // contrast: treatment (plugins loaded) vs control (empty tmpdir, no plugins).
