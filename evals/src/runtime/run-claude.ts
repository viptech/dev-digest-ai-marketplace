...
import { query, type Options, type SdkPluginConfig } from "@anthropic-ai/claude-agent-sdk";
...
export interface RunOptions {
  ...
  cwd?: string;
  model?: string;
  /** ["project"] loads on-disk CLAUDE.md + skills/agents; default [] keeps the run isolated. There
   * is no root CLAUDE.md / `.claude/` in this repo — workflowTask loads plugins via `plugins`
   * instead (see paths.ts's ALL_PLUGIN_CONFIGS), so this stays [] even for workflow cases. */
  settingSources?: Array<"user" | "project" | "local">;
  /** Local plugins to load for this session — the in-process equivalent of stacking
   * `--plugin-dir` flags. This is how workflowTask loads the four marketplace plugins without
   * any on-disk `.claude/` config. */
...
export async function runClaude(prompt: string, opts: RunOptions = {}): Promise<Result> {
  ...
  const options: Options = {
    ...
    permissionMode: "bypassPermissions", // skips the confirmation *prompt* only — see `tools` below
    // for the actual restriction. `Options.allowedTools` ("auto-allowed without prompting... to
    // restrict which tools are available, use the `tools` option instead" — sdk.d.ts) is NOT a
    // restriction on its own: verified live during this phase's spec-creator proof-of-concept run,
    // where an agent whose frontmatter granted Bash/Write/Edit (and whose computed allowedTools
    // correctly excluded them) still called Bash — because nothing had actually removed it from
    // the model's available tools. `Options.tools` is the field the SDK docs point to for that
    // ("[] disables all built-in tools"), so it's set here to the same allow-list.
    systemPrompt,
    tools: allowedTools,
    allowedTools,
