 ...
 *
 * Unlike the source engine (a single-namespace `.claude/skills` / `.claude/agents`), every
 * artifact here lives under a specific plugin — hence the `(plugin, name)` pair on every export.
 */

import { existsSync, readFileSync, readdirSync } from "node:fs";
import { join } from "node:path";
import { PLUGIN_DIRS } from "./paths.js";

function pluginDir(plugin: string): string {
  const dir = PLUGIN_DIRS[plugin];
  if (!dir) throw new Error(`unknown plugin: ${plugin} (known: ${Object.keys(PLUGIN_DIRS).join(", ")})`);
  return dir;
}
...
export function skillContent(plugin: string, skillName: string): string {
  const dir = join(pluginDir(plugin), "skills", skillName);
...
export function agentContent(plugin: string, agentName: string): string {
  const f = join(pluginDir(plugin), "agents", `${agentName}.md`);
  if (!existsSync(f)) throw new Error(`agent not found: ${f}`);
  return stripFrontmatter(readFileSync(f, "utf8"));
}

// Tools the eval refuses to hand a subagent: evals run with bypassPermissions against the plugin
// tree, so a mutating tool could take real actions. An agent that declares these still runs — it
 ...
 */
export function agentTools(plugin: string, agentName: string): string[] {
  const f = join(pluginDir(plugin), "agents", `${agentName}.md`);
  if (!existsSync(f)) throw new Error(`agent not found: ${f}`);
